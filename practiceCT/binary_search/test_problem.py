...
'''
-여행 가신 부모님을 대신해서 떡집 일을 하기로 하였다.
  오늘은 떡볶이 떡을 만드는 날이다.
  떡볶이 떡은 재밌게도 떡볶이 떡의 길이가 일정하지 않다.
...
- '현재 이 높이로 자르면 조건을 만족할 수 있는가?'를 확인한 뒤
  조건의 만족 여부('예' 혹은 '아니오')에 따라서 탐색 범위를 좁혀서 해결할 수 있다.
- 절단기의 높이는 0부터 10억까지의 정수 중 하나다.
    -->>이렇게 큰 탐색 범위를 보면 가장 먼저 이진 탐색을 떠올려야 한다!!!!!!!!!!!!!!!!!!!!!!!!!!
        선형탐색은 시간초과 판정을 받을 수 있어..

- 그렇게 이진 탐색 과정을 반복하면 답을 도출할 수 있다.
- 중간점의 값은 시간이 지날수록 '최적화된 값'이 되기 때문에,
  과정을 반복하면서 얻을 수 있는 떡의 길이 합이 필요한 떡의 길이보다
  크거나 같을 때마다 중간점의 값을 기록하면 된다.
'''
