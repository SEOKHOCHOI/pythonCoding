...
'''
ex)
  예를 들어 N = 5, K = 3이고, 배열 A와 B가 다음과 같다고 보자.
    배열 A = [1, 2, 5, 4, 3]
    배열 B = [5, 5, 6, 6, 5]
  이 경우, 다음과 같이 세 번의 연산을 수행할 수 있다.
    연산 1) 배열 A의 원소 '1'과 배열 B의 원소 '6'을 바꾸기
    연산 1) 배열 A의 원소 '2'과 배열 B의 원소 '6'을 바꾸기
    연산 1) 배열 A의 원소 '3'과 배열 B의 원소 '5'을 바꾸기
...
해결 아이디어
  ...
  이후에 두 배열의 원소를 첫 번째 인덱스부터 차례로 확인하며 A의 원소가 B의 원소보다 작을 때에만 교체를 수행한다.
  이 문제에서는 두 배열의 원소가 최대 1000,000개까지 입력될 수 있어, 최악의 경우 O(NlogN)을 보장하는 정렬 알고리즘을 이용해야 한다.
'''
